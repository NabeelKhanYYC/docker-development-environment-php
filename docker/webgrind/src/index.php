    ...
    switch (get('op')) {
        ...
        case 'function_graph':
            ...
            if (!file_exists($filename)) {
                ...
                // Is this a gunzip file?
                $dataFilePath = escapeshellarg(Webgrind_Config::xdebugOutputDir().$dataFile);
                $fileGZiped = false;
                if (strrpos($dataFile,".gz") !== false) {
                    shell_exec("/bin/gunzip -c ".$dataFilePath." > /tmp/".md5($dataFilePath));
                    if (file_exists("/tmp/".md5($dataFilePath))) {
                        $dataFilePath = "/tmp/".md5($dataFilePath);
                        $fileGZiped = true;
                    }
                }

                $CommandToExecute = Webgrind_Config::$pythonExecutable.' library/gprof2dot.py -n '.$showFraction
                           .' -f callgrind '.$dataFilePath.' | '
